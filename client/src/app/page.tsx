    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const sorted = apiInstructions.sort((a: Instruction, b: Instruction) =>
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-8 font-sans">
      <header className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Institutional Orchestration Control Plane</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          <MetricCard label="Total Volume" value={`$${totalVolume.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} color="blue" />
          <MetricCard label="Active Sagas" value={instructions.filter(i => i.state !== 'SETTLED' && i.state !== 'FAILED').length.toString()} color="yellow" />
          <MetricCard label="Settled 24h" value={instructions.filter(i => i.state === 'SETTLED').length.toString()} color="green" />
          <MetricCard label="Failed" value={instructions.filter(i => i.state === 'FAILED').length.toString()} color="red" />
        </div>
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-2xl">
          <div className="px-6 py-4 border-b border-gray-700 bg-gray-800/50 flex justify-between">
            ...
            <span className="text-xs text-gray-400 self-center">Polling /api/observe (2s)</span>
          </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-700">
                {instructions.map((inst) => (
                  <tr key={inst.instruction_id} className="hover:bg-gray-700/50 transition-colors">
                    <td className="px-6 py-4 font-mono text-gray-300">
                      <div className="font-bold text-white">{inst.instruction_id.slice(0, 8)}...</div>
                      <div className="text-xs text-gray-500">{inst.purpose}</div>
                    </td>
                ...
                {instructions.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500 italic">
                      No transactions found. Initiate payments to see them here.
                    </td>
                  </tr>
                )}
              </tbody>
...
function MetricCard({ label, value, color }: { label: string, value: string, color: 'blue' | 'green' | 'yellow' | 'red' }) {
  const colors = {
    blue: 'text-blue-400',
